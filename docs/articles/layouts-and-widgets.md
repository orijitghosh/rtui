...
rtui provides 35+ widgets and 9 layout containers. This guide covers
every one with practical examples you can copy and run.
...
Layouts are built by nesting container functions. Each container accepts
child widgets as arguments and arranges them according to its rules:
...
Every widget and container accepts optional `id` and `classes`
parameters. The `id` is used for event handling and updates; `classes`
are used for CSS styling.
...
You can set `rows`, `cols`, or both. Use CSS for fine-grained control
over column/row sizes:
...
An append-only scrolling log, ideal for status messages and debug
output:
...
The `language` parameter enables syntax highlighting. Supported
languages include `"r"`, `"python"`, `"javascript"`, `"markdown"`,
`"json"`, `"sql"`, and many more.
...
`>` (force uppercase following), `<` (force lowercase following). All
...
The `$accent`, `$surface`, `$text`, and `$text-muted` variables from
Textual are available in your CSS.
