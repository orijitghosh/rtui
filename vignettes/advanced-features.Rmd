...
This guide covers rtui's advanced capabilities: screens and modal dialogs,
timers and workers, key bindings, the command palette, structured forms, and
...
Screens let you build multi-page apps. You push a screen onto the stack to
show it, and pop it to return to the previous screen. Screens can return
...
Push a screen from any handler. The screen replaces the current view but the
main app is preserved underneath:
...
The screen is automatically dismissed when the user clicks `__dlg_yes` (result
`TRUE`) or `__dlg_no` (result `FALSE`). Handle the result:
...
For common cases, rtui provides `confirm()` and `alert()` which build and
push a styled dialog screen in one call:
...
Timers let you schedule actions -- updating a clock, polling data, running
animations, or implementing timeouts.
...
Workers are convenience wrappers around `set_interval()` for background
polling patterns:
...
- **`key`**: The key or key combination (`"q"`, `"ctrl+s"`, `"f1"`,
  `"escape"`, `"up"`, `"down"`, etc.)
- **`action`**: A string identifier dispatched as `event$value` in `on_action`
- **`description`**: Human-readable text shown in the footer
- **`priority`**: If `TRUE`, the binding fires even when a text input has
  focus. Use for global shortcuts like quit and save.
...
The command palette is a searchable pop-up menu opened with `Ctrl+P`. Register
custom commands that dispatch action events:
...
Commands are searched by name in the palette. When selected, the command's
`action` is dispatched to `on_action`:
...
`tui_form()` builds structured input forms with labelled fields and a submit
button:
...
3. Handle the button click with `on_click` and use `collect_form()` to gather
   all current values
...
Long-running computations block the terminal UI. `run_async()` solves this
by running R code in a separate background process via the `callr` package.
...
The function passed to `run_async()` must be **self-contained** — it runs
in a separate R process and cannot access variables from the calling session.
Pass data through the `args` parameter:
...
`event$timer_id` is the task name, so you can distinguish between multiple
concurrent tasks.
...
During development, `dev_app()` watches a `.R` file and automatically
restarts the app when the file changes:
...
2. Checks the file's modification time every second (configurable with `poll`)
3. When the file changes, kills the running app and restarts it
4. Continues until you press `Ctrl+C`

If the app crashes (syntax error, runtime error), `dev_app()` waits for
another file save before restarting — no need to manually restart the watcher.
...
The `log_view()` widget creates an append-only scrolling log, useful for
status messages, debug output, or activity feeds:
...
Here is a complete app that uses screens, timers, key bindings, the command
palette, and forms together:
...
> *before* the `quick_app()` call, because `quick_app()` runs the app
> immediately.
