---
title: "State, Events, and Reactivity"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{State, Events, and Reactivity}
...
This guide explains how rtui apps handle user interaction: the event system
that responds to clicks, key presses, and value changes; the mutable state
object that persists data across events; and the reactive binding system that
automatically updates widgets when state changes.
...
When an rtui app is running, Textual captures user input and dispatches events.
Each event is routed to an R callback function that you provide. The callback
inspects the event, optionally modifies state, updates widgets, and returns
state to continue.
...
- **`state`**: A mutable `RtuiState` object. Your persistent store across
  all events.
- **Return value**: Must return `state` (or `quit()` to exit).

## Available event types

Pass handlers to `tui_app()` or `quick_app()`:
...
Fires when a button is pressed or an item is clicked. `event$widget_id` tells
you which widget; `event$value` carries the click data.
...
Fires when the user presses Enter in an `input()` widget:
...
Fires when a key binding action is triggered. `event$value` is the action
name:
...
For `on_click`, `on_change`, and `on_submit`, you can pass a **named list**
instead of a single function. Each name is a widget id, and its value is the
handler for that specific widget:
...
The special `.default` key acts as a catch-all for widgets not explicitly
listed.
...
exits, `quick_app()` returns the final state invisibly.
...
Reactive bindings eliminate manual `update()` calls. Instead of this:
...
value is automatically updated. No manual `update()` needed.

### Binding types

The `reactive()` function accepts three binding forms:
...
In formulas, `.x` is the new value, `.state` is the state object, and `.app`
is the running app.
...
The `update()` function modifies a widget by id. Different widgets support
...
Return `quit()` from any handler to exit the app:
...
You can pass a result value that becomes the return value of `app$run()`:
...
Here's a complete app that demonstrates state, events, and reactive bindings
working together:
...
This example shows per-widget-id routing for both click and change events:
